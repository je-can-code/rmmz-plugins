...
/**
 * A constructor type that produces TInstance.
 * @template TInstance
 * @typedef {new (...args: any[]) => TInstance} Ctor
 */

/**
 * The synthetic $super object built by CycloneMovement.patchClass.
 * - If statics are overridden, instance methods live under `$prototype`.
 * - Otherwise, instance methods are copied directly on the root.
 *
 * @template {Ctor<any>} TCtor
 * @typedef {(
 *   Partial<TCtor> & (
 *     Partial<InstanceType<TCtor>> |
 *     { $prototype: Partial<InstanceType<TCtor>> }
 *   )
 * )} CycloneSuper
 */
(function()
{
  ...
  class CyclonePatcher
  {
    ...
    static _getAllClassDescriptors(classObj, usePrototype = false)
    {
      ...
      const descriptors = Object.getOwnPropertyDescriptors(usePrototype
        ? classObj.prototype
        : classObj);
    ...
    static _applyPatch(baseClass, patchClass, $super, ignoredNames, usePrototype = false)
    {
      ...
      const baseClassOrPrototype = usePrototype
        ? baseClass.prototype
        : baseClass;
      const patchClassOrPrototype = usePrototype
        ? patchClass.prototype
        : patchClass;
    ...
    static patchClass(baseClass, patchFn)
    {
      ...
      const ignoredNames = Object.getOwnPropertyNames((class Test
      {
      }).prototype);
    ...
    extends CyclonePatcher
  {
    ...
    static shouldReturnCallbackResult(result, {
      abortOnTrue,
      abortOnFalse,
      returnOnValue
    })
    ...
    })
    {
      ...
      {
        const itemData = this.parseStructParam({
          value: json,
          defaultValue: '',
          type
        });
      ...
      e = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+-$", t = {}, i = {
        ...
        decompressFromEncodedURIComponent: function(r)
        {
          return null == r
            ...
            : "" === r
              ? null
              : (r = r.replace(/ /g, "+"), i._decompress(r.length, 32, (n) =>
              {
        ...
        _compress: function(o, r, n)
        {
          ...
          {
            if (u = o.charAt(i), Object.prototype.hasOwnProperty.call(
              s,
              u) || (s[u] = f++, p[u] = !0), c = a + u, Object.prototype.hasOwnProperty.call(s, c))
            {
              a = c;
            }
            else
        ...
        _decompress: function(o, n, e)
        {
          ...
          {
            u = A.val & A.position, A.position >>= 1, 0 === A.position && (A.position = n, A.val = e(A.index++)), p |= (u > 0
              ? 1
              : 0) * a, a <<= 1;
          }
          switch (t = p)
  ...
  //#endregion really long function
  ...
  class CycloneMovement$1
    extends CyclonePlugin
  {
    ...
    static register()
    {
      ...
      this.stepCount = [ 1, 2, 4 ].includes(this.params.stepCount)
        ? this.params.stepCount
        : 1;
      this.collisionStepCount = Math.min(
        this.stepCount,
        [ 1, 2, 4 ].includes(this.params.collisionStepCount)
          ? this.params.collisionStepCount
          : 1);
      this.stepSize = 1 / this.stepCount;
      this.collisionSize = 1 / this.collisionStepCount;
      this.followerStepsBehind = Number(this.params.followerStepsBehind || 1)
        .clamp(1, this.stepCount);
    ...
    {
      const stepCount = useEditorStepCount
        ? 4
        : this.collisionStepCount;
  ...
  /**
   * @param {typeof Game_CharacterBase} classRef
   */
  const addPixelMovementToClass = (classRef) =>
  {
    /**
     * @typedef {(
     *   Partial<typeof Game_CharacterBase> & (
     *     Partial<Game_CharacterBase> | { $prototype: Partial<Game_CharacterBase> }
     *   )
     * )} GameCharacterBaseSuper
     */

    /**
     * @param {GameCharacterBaseSuper} $super
     * @param {Partial<Game_CharacterBase>} $prototype
     */
    CycloneMovement.patchClass(classRef, ($super, $prototype) => class
    {
      get left()
      {
      ...
      checkRightPassage(right, y, destinationRight, checkUp = false, checkDown = false)
      {
        ...
        if (lastXDestination > lastX)
        {
          ...
          const isLastXPassable = this.isPositionPassable(lastX, y, 6);
          if (!isLastXPassable)
          {
            return false;
          }
          ...
          const isLastXDestinationPassable = this.isPositionPassable(lastXDestination, y, 4);
          if (!isLastXDestinationPassable)
          {
            return false;
          }

          const canMoveVertically = this.checkVerticalPassage(lastXDestination, y, checkUp, checkDown) === false;
      ...
      isCollidedWithEvents(x, y)
      {
        return this.runForAllTiles(x, y, function(blockX, blockY)
        {
          ...
          return $gameMap.eventsXyNt(blockX, blockY)
            .some(event => event.isNormalPriority());
        });
      }
      ...
      chasePosition(x, y)
      {
        ...
        {
          this.moveStraight(sx > 0
            ? 4
            : 6);
        }
        else if (syAbs >= stepSize)
        {
          this.moveStraight(sy > 0
            ? 8
            : 2);
        }
      ...
      getDirectionNode(start, goalX, goalY)
      {
        ...
        while (nodeList.length)
        {
          ...
          for (let d = 1; d <= 9; d++)
          {
            ...
            const nextBest = this._findNextBestNode(
              best,
              x1,
              y1,
              d,
  ...
  CycloneMovement.patchClass(Game_Player, $super => class
  {
    ...
    {
      const passOptionA = this.canPass(this._x, this._y, direction);
      const passOptionB = (direction !== alternativeD && this.canPass(this._x, this._y, alternativeD));
      if (passOptionA || passOptionB)
      {
        this.onBeforeMove();
        ...
        if (!this.isMovementSucceeded())
        {
          ...
          // If none of the directions were clear and we were already facing one of them before
          // then revert back to it
          if (!this.isMovementSucceeded())
          {
    ...
    startMapEvent(x, y, triggers, normal)
    {
      ...
      const lastX = CycloneMovement.isRoundNumber(right)
        ? right - 1
        : Math.floor(right);
      const firstY = Math.floor(top);
      const lastY = CycloneMovement.isRoundNumber(bottom)
        ? bottom - 1
        : Math.floor(bottom);
    ...
    {
      const maxOffset = this.isInAirship()
        ? Math.ceil(CycloneMovement.stepCount / 2)
        : CycloneMovement.stepCount;
    ...
    getBestLandingPosition(vehicle, direction)
    {
      let x;
      let y;
      const vehicleX = this.x;
      const vehicleY = this.y;
      const { stepCount } = CycloneMovement;
