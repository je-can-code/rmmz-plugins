/**
 * Generated from project/js/rmmz_managers.js
 * Class: SoundManager
 * Instance/static typings merge with the engine constructor + prototype in project/js.
 * Do not hand-edit; regenerate with bun run defs:generate.
 * IDE: prototype navigation is authoritative in project/js/rmmz_*.js — ambient defs are for typing.
 */
declare function SoundManager(): never;
declare namespace SoundManager
{
  /**
   * Performs load system sound.
   * @param n The n parameter.
   */
  function loadSystemSound(n: unknown): void;
  /**
   * Performs play actor collapse.
   */
  function playActorCollapse(): void;
  /**
   * Performs play actor damage.
   */
  function playActorDamage(): void;
  /**
   * Performs play battle start.
   */
  function playBattleStart(): void;
  /**
   * Performs play boss collapse1.
   */
  function playBossCollapse1(): void;
  /**
   * Performs play boss collapse2.
   */
  function playBossCollapse2(): void;
  /**
   * Performs play buzzer.
   */
  function playBuzzer(): void;
  /**
   * Performs play cancel.
   */
  function playCancel(): void;
  /**
   * Performs play cursor.
   */
  function playCursor(): void;
  /**
   * Performs play enemy attack.
   */
  function playEnemyAttack(): void;
  /**
   * Performs play enemy collapse.
   */
  function playEnemyCollapse(): void;
  /**
   * Performs play enemy damage.
   */
  function playEnemyDamage(): void;
  /**
   * Performs play equip.
   */
  function playEquip(): void;
  /**
   * Performs play escape.
   */
  function playEscape(): void;
  /**
   * Performs play evasion.
   */
  function playEvasion(): void;
  /**
   * Performs play load.
   */
  function playLoad(): void;
  /**
   * Performs play magic evasion.
   */
  function playMagicEvasion(): void;
  /**
   * Performs play miss.
   */
  function playMiss(): void;
  /**
   * Performs play ok.
   */
  function playOk(): void;
  /**
   * Performs play recovery.
   */
  function playRecovery(): void;
  /**
   * Performs play reflection.
   */
  function playReflection(): void;
  /**
   * Performs play save.
   */
  function playSave(): void;
  /**
   * Performs play shop.
   */
  function playShop(): void;
  /**
   * Performs play system sound.
   * @param n The n parameter.
   */
  function playSystemSound(n: unknown): void;
  /**
   * Performs play use item.
   */
  function playUseItem(): void;
  /**
   * Performs play use skill.
   */
  function playUseSkill(): void;
  /**
   * Performs preload important sounds.
   */
  function preloadImportantSounds(): void;
}
