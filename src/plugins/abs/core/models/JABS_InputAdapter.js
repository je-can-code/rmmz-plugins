...
class JABS_InputAdapter
{
  ...
  static performUsableItemAction(jabsBattler)
  {
    ...
    // grab the item id currently equipped in the usable-item slot.
    const itemId = jabsBattler.getBattler()
      .getEquippedSkillId(JABS_Button.UsableItem);

    // perform usable-item effects.
  ...
  {
    // if the usable-item slot is not off cooldown, then do not perform.
    if (!jabsBattler.isSkillTypeCooldownReady(JABS_Button.UsableItem)) return false;

    // if there is no item equipped in the usable-item slot, then do not perform.
    if (!jabsBattler.getBattler()
      .getEquippedSkillId(JABS_Button.UsableItem))
    {
      return false;
