...
{
  // validate we have an enemy to be named after.
  if (this.enemy())
  {
    // enemies share their database ids with every other copy of themselves, so the generated uuid
    // has to come along to tell two of the same enemy apart.
    return `${this.name()}_${this.uuid()}`;
  }

  // an enemy still being constructed has no identity to offer, and no tracked states to find by it.
  return String.empty;
};
