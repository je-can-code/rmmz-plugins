...
Game_Party.prototype.initMembers = function()
{
  ...
  J.SDP.Aliased.Game_Party.get('initMembers')
    .call(this);
...
Game_Party.prototype.unlockSdp = function(key)
{
  ...
  {
    // stop processing.
    Diagnostics.error(__PLUGIN_NAME__, `the SDP key of ${key} was not found in the list of panels to unlock.`);
    return;
  }
...
Game_Party.prototype.lockSdp = function(key)
{
  ...
  {
    // stop processing.
    Diagnostics.error(__PLUGIN_NAME__, `the SDP key of ${key} was not found in the list of panels to lock.`);
    return;
  }
...
Game_Party.prototype.getSdpRankByActorAndKey = function(actorId, key)
{
  ...
  if (!actor)
  {
    Diagnostics.error(__PLUGIN_NAME__, `the actor id of ${actorId} was invalid.`);
    return 0;
  }
