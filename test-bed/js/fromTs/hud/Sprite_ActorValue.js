 ...
 */
class Sprite_ActorValue extends Sprite {
    constructor(actor, type, fontSizeMod) {
        super(null);
        this.setup(actor, type, fontSizeMod);
    }
    setup(actor, parameter, fontSizeMod = 0) {
        this._j = {};
    ...
    hasParameterChanged() {
        ...
        switch (this._j._parameter) {
            case "hp":
                ...
                if (changed)
                    this._j._last._hp = this._j._actor.hp;
                return changed;
            case "mp":
                changed = this._j._actor.mp != this._j._last._mp;
                if (changed)
                    this._j._last._mp = this._j._actor.mp;
                return changed;
            case "tp":
                changed = this._j._actor.tp != this._j._last._tp;
                if (changed)
                    this._j._last.tp = this._j._actor.tp;
                return changed;
            case "xp":
                changed = this._j._actor.currentExp() != this._j._last._xp;
                if (changed)
                    this._j._last._xp = this._j._actor.currentExp();
                return changed;
            case "lvl":
                changed = this._j._actor.level != this._j._last._lvl;
                if (changed)
                    this._j._last._lvl = this._j._actor.level;
                return changed;
        }
    }
    /**
