...
describe('J-CMS-Skill metadata (direct src import)', () =>
{
  ...
  {
    // Arrange & Act
    const { Aliased } = globalThis.J.CMS_K;
  ...
  {
    // Arrange & Act & Assert- the skill extension and the menu core are separately installable, so
    // each owns its own umbrella.
    expect(globalThis.J.CMS_K).toBeDefined();
    expect(globalThis.J.CMS_K.Metadata.parsedPluginParameters).toBeDefined();
  });
