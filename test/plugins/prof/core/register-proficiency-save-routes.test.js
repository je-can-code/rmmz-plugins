//region plugins/prof/core/register-proficiency-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerProficiencySaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/prof/core/registerProficiencySaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the proficiency namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_proficiency')).toBe('systems/proficiency.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_proficiency')).toBe(false);
  });
});
//endregion plugins/prof/core/register-proficiency-save-routes.test.js