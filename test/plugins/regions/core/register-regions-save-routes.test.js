//region plugins/regions/core/register-regions-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerRegionsSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/regions/core/registerRegionsSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the regions namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_regions')).toBe('systems/regions.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_regions')).toBe(false);
  });
});
//endregion plugins/regions/core/register-regions-save-routes.test.js