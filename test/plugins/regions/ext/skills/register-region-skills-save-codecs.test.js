//region plugins/regions/ext/skills/register-region-skills-save-codecs.test.js
import { beforeEach, describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../../setup/install-save-registration-realm.js';

describe('registerRegionSkillsSaveCodecs', () =>
{
  /**
  ...
  /**
   * Reads the execution timer factory off a host's declarations.
   * @param {Function} host The constructor the declaration was filed against.
   * @returns {Function} The factory producing a cold execution timer.
   */
  const timerFactory = host => realm.SerializableRegistry.registrations()
    .get(host)
    .transients['_j._regions._skills._timer'];
  ...
  beforeEach(async () =>
  {
    realm = await installSaveRegistrationRealm({
      ...
      globals: { JABS_Timer },
      namespaces: { REGIONS: { EXT: { SKILLS: { Metadata: { delayBetweenExecutions: 24 } } } } },
    });

    await import('../../../../../src/plugins/regions/ext/skills/registerRegionSkillsSaveCodecs.js');
  });

  it('declares the execution timer transient on all three character-like hosts', () =>
  {
    // Arrange
  ...
  it('builds the cold timer from the current plugin parameter, not from a written delay', () =>
  {
    ...
    // Assert
    expect(timer.duration).toBe(24);
  });
});
//endregion plugins/regions/ext/skills/register-region-skills-save-codecs.test.js