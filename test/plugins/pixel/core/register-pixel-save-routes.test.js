//region plugins/pixel/core/register-pixel-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerPixelSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/pixel/core/registerPixelSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the pixel namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_pixel')).toBe('systems/pixel.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_pixel')).toBe(false);
  });
});
//endregion plugins/pixel/core/register-pixel-save-routes.test.js