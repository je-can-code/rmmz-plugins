//region plugins/diff/core/register-difficulty-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerDifficultySaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/diff/core/registerDifficultySaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the difficulty namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_difficulty')).toBe('systems/difficulty.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_difficulty')).toBe(false);
  });
});
//endregion plugins/diff/core/register-difficulty-save-routes.test.js