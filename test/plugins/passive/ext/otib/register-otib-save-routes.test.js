//region plugins/passive/ext/otib/register-otib-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../../setup/install-save-registration-realm.js';

describe('registerOtibSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../../src/plugins/passive/ext/otib/registerOtibSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the otib namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_otib')).toBe('systems/otib.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_otib')).toBe(false);
  });
});
//endregion plugins/passive/ext/otib/register-otib-save-routes.test.js