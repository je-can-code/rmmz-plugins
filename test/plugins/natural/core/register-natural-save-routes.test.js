//region plugins/natural/core/register-natural-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerNaturalSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/natural/core/registerNaturalSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the natural namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_natural')).toBe('systems/natural.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_natural')).toBe(false);
  });
});
//endregion plugins/natural/core/register-natural-save-routes.test.js