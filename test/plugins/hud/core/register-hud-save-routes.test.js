...
describe('registerHudSaveRoutes', () =>
{
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/hud/core/registerHudSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the hud namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_hud')).toBe('systems/hud.json');
  });
...
//endregion plugins/hud/core/register-hud-save-routes.test.js