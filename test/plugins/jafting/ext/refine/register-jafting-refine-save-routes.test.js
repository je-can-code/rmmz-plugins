//region plugins/jafting/ext/refine/register-jafting-refine-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../../setup/install-save-registration-realm.js';

describe('registerJaftingRefineSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../../src/plugins/jafting/ext/refine/registerJaftingRefineSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the refinement namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_refinement')).toBe('systems/refinement.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_refinement')).toBe(false);
  });
});
//endregion plugins/jafting/ext/refine/register-jafting-refine-save-routes.test.js