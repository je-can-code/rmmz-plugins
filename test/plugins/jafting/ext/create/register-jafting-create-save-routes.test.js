//region plugins/jafting/ext/create/register-jafting-create-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../../setup/install-save-registration-realm.js';

describe('registerJaftingCreateSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../../src/plugins/jafting/ext/create/registerJaftingCreateSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the crafting namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_crafting')).toBe('systems/crafting.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_crafting')).toBe(false);
  });
});
//endregion plugins/jafting/ext/create/register-jafting-create-save-routes.test.js