//region plugins/jafting/core/register-jafting-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerJaftingSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/jafting/core/registerJaftingSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the jafting namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_jafting')).toBe('systems/jafting.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_jafting')).toBe(false);
  });
});
//endregion plugins/jafting/core/register-jafting-save-routes.test.js