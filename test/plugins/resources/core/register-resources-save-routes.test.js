//region plugins/resources/core/register-resources-save-routes.test.js
import { describe, expect, it } from 'vitest';

import { installSaveRegistrationRealm } from '../../../setup/install-save-registration-realm.js';

describe('registerResourcesSaveRoutes', () =>
{
  /**
  ...
  const importWithSaveExtension = async saveExtensionInstalled =>
  {
    ...
    await import('../../../../src/plugins/resources/core/registerResourcesSaveRoutes.js');

    return SaveSectionRouter;
  };

  it('routes the resources namespace into its own section file when J-Base-Save is installed', async () =>
  {
    // Arrange
    // Act
    ...
    expect(SaveSectionRouter.routedNamespaces()
      .get('_resources')).toBe('systems/resources.json');
  });
  ...
  it('routes nothing when J-Base-Save is absent, leaving the slice inline on its host', async () =>
  {
    ...
    // Assert
    expect(SaveSectionRouter.routedNamespaces()
      .has('_resources')).toBe(false);
  });
});
//endregion plugins/resources/core/register-resources-save-routes.test.js