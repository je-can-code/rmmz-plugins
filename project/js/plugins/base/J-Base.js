...
var Diagnostics = class Diagnostics {
	...
	*/
	static emit(channel, pluginName, message, details) {
		...
		const text = resolvedMessage.error === null ? resolvedMessage.value : Diagnostics.MESSAGE_BUILD_FAILURE;
...
//#endregion
//#region src/plugins/_base/core/core/TextWrapper.js
/**
...
var TextWrapper = class TextWrapper {
	...
	*/
	static wrap(text, maxWidth, measure) {
		...
		words.forEach((word) => {
			const candidate = current === String.empty ? word : `${current} ${word}`;
